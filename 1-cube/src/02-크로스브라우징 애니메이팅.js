...
function init() {
    ...
    function render() {
        ...
        // cube.rotation.x += 0.01; // 이렇게 할경우 fps 피씨마다 달라서 다르게 보일 수 있음

        //방법1. date객체
        cube.rotation.x = Date.now() / 1000;
        //clock 객체
        //방법3. getElapsedTime() - clock객체 생성 이후 흐른 시간
        cube.rotation.x = clock.getElapsedTime();

        //방법3. getDelta() - 절대적 시간이 아닌 delta가 호출되는 간격
        cube.rotation.x += clock.getDelta();

        // +1, -1 사이를 오가서 위아래로 움직임 (sin cos 그래프 모양대로)
