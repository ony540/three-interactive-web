...
function init() {
  ...
  const gui = new GUI();
  // gui.add(cube.position, "y", -3, 3, 0.1); //cube.position의 y축값 조정, 범위 -3~ 3 , 단계 0.1
  ...
  // 색상제어의 경우 다른 매서드
  gui
    .addColor(options, 'color') //options객체안의 color의 값을 바꿀수있는 color패널이 나온다
    .onChange((value) => {
      cube.material.color.set(value);
    }); //onChange 으로 해당 value로 색을 지정한다
}
